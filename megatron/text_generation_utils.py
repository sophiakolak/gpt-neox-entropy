...
def stream_tokens(neox_args, model, context_tokens: List[List[int]], eos_token_id: int = None, 
    ...
    with torch.no_grad():
        ...
        max_logit_predicted_tokens = []
        sequence_entropy = torch.Tensor().cuda()
        ...
        while token_index_to_generate <= last_token_index_to_generate:
            ...
            if temperature == 0.0 and top_k == 0 and top_p == 0.0:
                ...
                token_prob = next_token_log_probs[:, generated_tokens]
                entropy = torch.neg(torch.log(token_prob))
                sequence_entropy = torch.cat([sequence_entropy, entropy], dim=0)

                index = generated_tokens.tolist()[0]
                argmax = torch.argmax(next_token_log_probs)

                torch.set_printoptions(precision=10)
                prob = next_token_log_probs[0,index]
                prob_item = next_token_log_probs[0,index].item()
            ...
            yield context_tokens, token_generation_start_index, token_generation_end_index, all_logits, state_is_done.bool(), sequence_entropy
...
def generate_samples_from_prompt(neox_args, model, text: Union[List[str], str], eos_token_id: int = None, 
    ...
    while True:
        ...
        for batch_context_tokens, batch_token_generation_start_index, batch_token_generation_end_index, batch_logits, is_done, sequence_entropy in stream_tokens(
            neox_args=neox_args, 
            model=model,
        ...
        for tokens, start_index, end_index, is_done, logits in zip(batch_context_tokens, batch_token_generation_start_index, batch_token_generation_end_index, batch_is_done, batch_logits):
            if end_index >= start_index:
                generated_tokens = tokens[start_index:end_index + 1]
                generated_logits = logits[start_index:end_index + 1]
                try:
                    index = 0
                    for token in generated_tokens:
                        token_string = neox_args.tokenizer.detokenize([token])
                        if '\n' in token_string:
                            break
                        index+=1
                    first_newline_index = index+1
                    sequence_entropy_sum = torch.sum(sequence_entropy[:first_newline_index])
                    entropy = sequence_entropy_sum.item()

                    generated_text = neox_args.tokenizer.detokenize(generated_tokens[:first_newline_index])
                    message = None
                except KeyError:
                    generated_text = None
...
def generate_samples_input_from_file(neox_args, model, input_file, output_file=None, eos_token_id: int = None, 
    ...
    #print("number of samples", neox_args.num_samples)
    ...
    for item in generated_texts:
        ...
        index_entropy[i] = entropy
        i += 1
    
    sorted_results = dict(sorted(index_entropy.items(), key=lambda item: item[1]))
    ranked_results = []
    for index in sorted_results:
        result = generated_texts[index]
        ranked_results.append(generated_texts[index])
        
    if is_mp_rank_0():
        with open(output_file, "w") as f_out:
            for item in ranked_results:
                #print_rank_0(item['text'])
