...
def main():
    ...
    if neox_args.text_gen_type == 'unconditional':
        ...
        count = 1
        for file in os.listdir("py-bugs"):
            if file.endswith(".py"):
                python_program = os.path.join("py-bugs", file)
                if "_test.py" in python_program:
                    continue
                neox_args.sample_input_file = python_program 
                neox_args.sample_output_file = "py-bugs-output/"+file
                print("input=",neox_args.sample_input_file, "output=",neox_args.sample_output_file)
                print("program ", count, "/37")
